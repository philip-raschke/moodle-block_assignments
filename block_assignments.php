...
class block_assignments extends block_base {
    ...
    public function get_content() {
		...
		$linkInHead = '';		
		$graded_assignments_output = '';
		
		if(empty($assign_grades))
			$graded_assignments_output .= no_data_row(2);
		else {
			...
			foreach($assign_grades as $entry) {
				...
				if(course_too_old($course)) {
					if($counter == count($assign_grades))
						$graded_assignments_output .= no_data_row(2);					
					continue;
				}
				...
				$graded_assignments_output .= html_writer::tag('tr', $name.$grade, $displayNone);
			}
		}
		if($overflow)
			$linkInHead = html_writer::tag('a', '<b>'.get_string('more', 'block_assignments').'</b>', array('class' => 'btn-show-all btn-show-all-right', 'href' => new moodle_url('/blocks/assignments/view.php')));
		...
		$th_name = html_writer::tag('th', get_string('graded', 'block_assignments').$linkInHead, array('colspan' => '2'));
		$this->content->text .= html_writer::tag('thead', $th_name);		
		$this->content->text .= $graded_assignments_output;
		...
		$linkInHead = '';
		$open_assignments_output = '';
		
		if(empty($assignments))
			$open_assignments_output .= no_data_row(2);
		else {		
			...
			foreach($assignments as $assignment) {
				...
				if(!in_array($assignment->id, $graded_assignments)) {
					...
					$open_assignments_output .= html_writer::tag('tr', $name.$due, $displayNone);
				}
				else
		...
		$th_name = html_writer::tag('th', get_string('open_assignments', 'block_assignments').$linkInHead, array('colspan' => '2'));
		$this->content->text .= html_writer::tag('thead', $th_name);
		$this->content->text .= $open_assignments_output;
		$this->content->text .= html_writer::end_tag('table');
		$this->content->text .= html_writer::end_tag('div');
		$this->content->text .= html_writer::end_tag('div');
